...
const translations = {
  en: {
    ...
    'proj.misinfo.desc': 'NLP-based fake news detection system classifying 5,284 tweets (COVID-19, Refugees, Russia-Ukraine) into True/False/Other. Engineered 4,000+ features across 5 categories: social media interactions, deep URL analysis (domain type classification), linguistic patterns (caps, profanity, adjective ratios), TF-IDF (1,480 features), and sentiment/subjectivity scores. Models: Random Forest, XGBoost, SVM — best result SVM with 77% accuracy, 0.88 AUC, 0.73 F1. <strong>Best Technology Award.</strong>',
    'proj.website.title': 'Personal Website with Dual AI Chatbot',
    'proj.website.desc': 'Full-stack personal portfolio with an integrated AI assistant powered by a self-built RAG pipeline. Embedding: Silicon Flow BGE-M3 (1024-dim bilingual vectors). Vector DB: Supabase pgvector with cosine similarity search over 1,000+ knowledge chunks. Generation: DeepSeek V3 with multi-turn conversation support. Frontend: bilingual (EN/CN), dark/light theme. API: Vercel Edge Functions. The entire site and RAG system were built from scratch.',
  ...
  zh: {
    ...
    'proj.misinfo.desc': '基于 NLP 的假新闻检测系统，对 5,284 条推文（COVID-19、难民、俄乌战争）进行三分类。构建 4,000+ 维特征，涵盖 5 大类：社交互动特征、深度 URL 分析（域名类型分类）、语言学模式（大写比例、脏话检测、形容词比率）、TF-IDF（1,480 维）、情感/主观性评分。模型：随机森林、XGBoost、SVM——最佳 SVM 准确率 77%，AUC 0.88，F1 0.73。<strong>最佳技术奖。</strong>',
    'proj.website.title': '个人网站 · 双 AI Chatbot',
    'proj.website.desc': '全栈个人作品集网站，集成自建 RAG 管线驱动的 AI 助手。向量化：Silicon Flow BGE-M3（1024 维中英双语向量）。向量库：Supabase pgvector，余弦相似度检索 1,000+ 知识块。生成：DeepSeek V3，支持多轮对话。前端：中英双语、明暗主题。API 层：Vercel Edge Functions。全站及 RAG 系统从零独立构建。',
